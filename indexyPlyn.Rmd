...
REQ:
  - aktualni CSV soubory musi byt stazene v "X:/ICIS_Heren/
  
FUNC:
  - tato sekce kodu zkopiruje soubor indexyPlyn_AKTUALNI.xlsx do slozky R_indexHarvest, kde budou probihat upravy
  - CSV se nakopiruji do R_indexHarvest/csv_oper
  - probehne sklizeni vsech CSV souboru a zapis do indexyPlyn_AKTUALNI.xlsx na listy ICIS a ICIS komplet
...
# paths
path_in <- "X:/Nakup _ NEW/Indexy_Plyn/R_indexHarvest/csv_oper/" 
path_bu <- "X:/Nakup _ NEW/Indexy_Plyn/R_indexHarvest/d-1_backup/"
path_out <- "X:/Nakup _ NEW/Indexy_Plyn/R_indexHarvest/"
...
# ------------------------------------- copy excel from X to local


file.copy(from = "X:/Nakup _ NEW/Indexy_Plyn/indexyPlyn_AKTUALNI.xlsx",
          to = paste0(path_out, "indexyPlyn_AKTUALNI.xlsx"),
          overwrite = TRUE)
...
for (i in Ltrh) {
  ...
  if (length(files) > 0) {
    ...
    df <- read.csv(newest_file, header = TRUE, sep = ",") %>% 
      ...
      # mutate(publ = as.Date(Date, format = "%d %b %Y"), # zmena formatu csv 13.1.2026
      mutate(publ = as.Date(Published, format = "%d %b %Y"),
...
print("Dnesni davka indexu zapsana do lokalu.")
```


# FINAL SAVE AND BACKUP

REQ:
  - zapis aktualnich indexu do R_indexHarvest/indexyPlyn_AKTUALNI.xlsx probehl uspesne
  - problehla kontrola uspesnosti zapisu dat
  
FUNC:
  - excel se kopiruje zpet do slozky Indexy_Plyn
  - zaroven se vytvori zaloha v R_indexHarvest/d-1_backup
  
*WARN!:*
  *- v pondeli se tato uloha spousti az po zapsani tydenni davky indexu z EEX*
...
print("Dnesni davka indexu zapsana na Xko.")
...
FUNC:
  - aktualizuje graf vyvoje cen DA za posledni tri mesice a *vymaze vsechny docasne soubory z Environment*
...
print("Graf vyvoje DA aktualizovan.")
...
REQ:
  - na strankach EEX pracujeme s nasledujicimi sekcemi
    - NG -> SPOT -> CZVTP -> DA
    - NG -> SPOT -> THE -> DA
    - NG -> SPOT -> TTF -> DA
  - v rozsahu datumu zvolit minuly tyden
  - na tabulku klikneme PM -> Prozkoumat (otevre zdrojovy kod) -> o par radku vys  <body>, na ktere klikneme PM -> Upravit HTML -> zkopirovat cely text do clipboardu -> spustit od
  - projet trikrat vzdy pro kazdy trh (treba odkomentovat a zase zakomentovat "market")
  
FUNC:
  - vytvori tri samostatne dataframe, pro kazdy trh jeden 
  
*WARN!:*
  *- WE hodnoty se nevyplnuji, je treba je doplnit do excelu po skonceni cele teto R akce rucne.*
  *- najdeme zde: - NG -> SPOT -> CZVTP -> WE*
...
REQ:
  - Environment obsahuje 6 dataframes
    - eod_vtp
    - eod_the
    - eod_ttf
    - egsi_vtp
    - egsi_the
    - egsi_ttf
  *- v poslednim radku je treba nastavit spravne datum*
  
FUNC:
  - zapise EoD a EGSI do excelu na list EEX (treba odkomentovat a zase zakomentovat)
...
eex <- egsi_ttf %>% 
  ...
  filter(del_date > "2026-02-23") # --------------------------- uprav - minule pondeli

# EOD a EGSI
# append_to_excel(eex, paste0(path_out, "indexyPlyn_AKTUALNI.xlsx"), sheet = "EEX") # uncomment to append
...
REQ:
  - naplneny operacni textak R_indexHarvest/oper_EEXfutures.txt
  - veta by mela obsahovat zdrojovy kod pro tyto sekce:
      VTP Cal+1 -> VTP Q+1 -> VTP M+1 -> 
      THE Cal+1 -> THE Q+1 -> THE M+1 -> 
      TTF Cal+1 -> TTF Q+1 -> TTF M+1
  - trading day == minule pondeli
  
FUNC:
  - zpracuje txt do tabulky se vsemi futures -> zkontrolovat

  
```{r}

text <- readLines(paste0(path_out, "oper_EEXfutures.txt")) # warning is fine, warn = FALSE
text <- paste(text, collapse = "") # vymaze pripadne mezery v HTML
...
REQ:
  - zkontrolovany dataframe futu
  
FUNC:
  - zapise do excelu na list EEX_futures
...
# write.table(futu, "clipboard", sep = "\t", row.names = FALSE)

```

*TO DO*
  - po zapsani EEX futures - zkontrolovat excel
  - prejit do sekce FINAL SAVE AND BACKUP a projet vytvoreni zalohy a zkopirovani excelu do Indexy_Plyn
